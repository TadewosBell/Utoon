...
export class Sky extends ex.Actor {
  ...
  createStars() {
    ...
    this.stars = new Array(amount).fill(0).map((_, i) => {
      const column = bounds.left + (i % Math.floor(bounds.width / gap)) * gap + (Math.random() - 0.5) * gap;
      const row = bounds.top + Math.floor(i / Math.floor(bounds.width / gap)) * gap + (Math.random() - 0.5) * gap;
...
class Star extends ex.Actor {
  constructor(args) {
    ...
    super({ z: -4, scale: ex.vec(scale, scale), ...args });
  }

  onInitialize() {
    const spritesheet = ex.SpriteSheet.fromImageSource({
      image: $res("sprites/levels/star.png"),
      grid: { columns: 3, rows: 1, spriteWidth: 5, spriteHeight: 5 },
    });
    const anim = ex.Animation.fromSpriteSheet(spritesheet, [0, 1, 2], 400, ex.AnimationStrategy.PingPong);
